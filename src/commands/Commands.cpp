...
void Commands::execute(Client *client, std::string Command)
{
	...
	{
		Message err(*client, "");
		(client->_client_user.connected) ? err.set_message_error(ERR_UNKNOWNCOMMAND(this->_server->serverName, client->_client_user.nickname, name)) : err.set_message_error(ERR_NOTREGISTERED(this->_server->serverName, name));
